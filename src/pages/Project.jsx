...
export default function Project() {
  ...
  useEffect(() => {
    let pending = 0;
    if (formData && Array.isArray(formData?.items) && formData?.items[0]) {
      pending =
        Number(formData.items[0].receivedQuantity) -
        Number(formData.items[0].installedPanels);
      setPending1(pending);
    } else {
  ...
  return (
    <>
      <div className="lg:ms-[18%] lg:w-[82%] bg-[#F4F6F8] w-full  ">
        <div className="w-full flex md:px-10 px-2  pb-1 pt-14  bg-white flex-col">
