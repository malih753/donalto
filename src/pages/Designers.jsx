...
export default function Designers() {
  ...
  const handleChange4 = ({ fileList: newFileList }) => {
    setFileList4(newFileList); // Update fileList3 regardless of isFileAllowed
  };
  const handleChange5 = ({ fileList: newFileList }) => {
    setFileList5(newFileList); // Update fileList3 regardless of isFileAllowed
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData2.append("file", fileList[0]?.originFileObj);
      console.log("file", fileList[0]);

      console.log(commentFile[0]?.originFileObj);

      const commentFormData = new FormData();

      if (commentFile[0]?.originFileObj) {
        commentFormData.append("commentFile", commentFile[0]?.originFileObj);
      }
                ...
                comments: [
                  {
                    ...
                    commentMedia: commentFile[0]?.originFileObj,
                  },
                ],
              },
            ],
  ...
  const updateFile = async () => {
    try {
      const filePromise = projects?.map(async (project) => {
        if (!project?.layouts[0]?.file[0]?.originFileObj) {
          return;
        }
        const formData = new FormData();

        console.log(
          "project?.layouts[0]?.file[0]?.originFileObj => ",
          project?.layouts[0]?.file[0]?.originFileObj
        );
        formData.append("file", project?.layouts[0]?.file[0]?.originFileObj);
        const fileRes = await axios.post(
          `${serverDomain}/upload/file`,
          formData,
          {
      ...
      const fileRes = await Promise.all(filePromise);
      console.log("fileRes => ", fileRes);

      const urls = fileRes?.map((item) => item?.data?.file);
      ...
      const data = projects?.map((item, index) => ({
        ...
        layouts: item?.layouts?.map((layout) => ({
          ...layout,
          fileUrl: (urls && urls[index]) || layout?.fileUrl,
        })),
      }));
      ...
      const res = await Promise.all(projectPromises);
      console.log("update product response", res);
  ...
  const generatePDF = () => {
    ...
    doc.autoTable({
      head: [["Pannel Name", "Comment Text", "Comment Date"]],
      body: tableData,
    });
      ...
      try {
        ...
        setProjects(
          res.data.data.map((item) => ({
            ...item,
            layouts: item?.layouts?.map((layout) => ({ ...layout, file: [] })),
          }))
        );
        // setFileList(res.data.data.map((item) => item.fileUrl));
      } catch (error) {
        console.log(error);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.get(
          `${serverDomain}/boqSummary/projectById/${projectId}`,
          {
            headers: {
              Authorization: authToken,
            },
        ...
        <div className="w-full  flex flex-row 2xl:flex-nowrap gap-3 flex-wrap items-center border p-3 rounded-xl border-[rgba(0, 0, 0, 0.10)] justify-between">
          ...
          <div className=" xl:w-[60%] w-full flex flex-row sm:flex-nowrap flex-wrap  gap-4 justify-start md:justify-end">
            ...
            >
              <button className=" w-full items-center justify-center   md:px-5 md:h-[38px] h-[50px] flex  font-medium rounded-lg text-white  flex-row gap-1 ">
                <AiOutlinePlus className="text-white text-lg" />
                <span className=" text-sm">Update Pannel</span>
              </button>
            </div>
          </div>
        </div>
                ...
                <>
                  <div className="min-h-[1000px] min-w-[435px] flex flex-col p-2 gap-3 bg-white border rounded-xl border-[rgba(0, 0, 0, 0.20)]">
                    ...
                    <div className=" p-2 rounded-lg flex flex-col gap-2 bg-[#F7F7F7] mt-1">
                      ...
                      <div className=" w-[30%] bg-[#D3E9FF] p-2 rounded-lg">
                        <Upload
                          listType="picture-card"
                          beforeUpload={beforeUpload}
                          fileList={p?.layouts?.[0]?.file || []} // Use optional chaining to safely access layouts[i]
                          onChange={({ fileList }) => {
                            console.log("fileList", fileList);
                            setProjects((prev) =>
                              prev.map((proj) => {
                        ...
                        >
                          {p?.layouts?.[0]?.file?.length >= 1
                            ? null
                            : uploadButton}{" "}
                          {/* Safely check file length */}
                        </Upload>
                      ...
                      <div className=" flex justify-between gap-2 items-start w-full ">
                        ...
                        >
                          ...
                          <Link
                            to={`${serverDomain}/${p?.layouts[0].fileUrl}`}
                            className="gap-2 w-[70%] flex items-center bg-[#D3E9FF] p-2 rounded-lg"
                          >
                            {/* <Upload
                            listType="picture-card"
                            beforeUpload={beforeUpload}
                          ...
                          >
                            ...
                            />
                            <p className=" text-[#828282] text-xs">
                              {/* {p?.layouts[0].fileUrl?.split("/").pop()} */}
                              {typeof p?.layouts[0].fileUrl === "string"
                                ? p?.layouts[0].fileUrl?.split("/")?.pop()
                                : p?.layouts[0].fileUrl}
                            </p>
                          </Link>
                    ...
                    <div className=" p-2 rounded-lg flex flex-col gap-2 bg-[#F7F7F7] mt-1">
                      ...
                      <div className=" flex flex-row justify-between gap-2 items-start w-full ">
                        ...
                        >
                          ...
                          <img src={"/file.png"} className="w-8 h-8 " alt="" />
                          <p className=" text-[#828282] text-xs">
                            {typeof p?.layouts[0].fileUrl === "string"
                              ? p?.layouts[0].fileUrl?.split("/")?.pop()
                              : p?.layouts[0].fileUrl}
